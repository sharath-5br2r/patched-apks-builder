...
dolphin() {
    ...
    DOLPHIN_APK_URL=$( curl -s https://dolphin-emu.org/download/ -H "Cookie: $FS_COOKIES" -H "User-Agent: $user_agent" | grep -Eo 'https://dl\.dolphin-emu\.org/builds/[a-z0-9/]+/dolphin-master-[0-9]+-[0-9]+\.apk' | awk -F'[-/.]' '{v=$(NF-2); b=$(NF-1);if (v>V || (v==V && b>B)) {V=v; B=b; U=$0}} END{print U}')
