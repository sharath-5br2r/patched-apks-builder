...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		echo " java -jar *cli*.jar  $p$b --keystore-password=$KEYSTORE_PASS --keystore-entry-password=$KEYSTORE_PASS --keystore-entry-alias="sharath" $m$opt --out=./release/$1-$2.apk$excludePatches$includePatches$ks $pu$force $a./download/$1.apk "
  		unset version
		unset lock_version
