...
patch() {
	...
	if [ -f "./download/$1.apk" ]; then
		...
		eval java -jar *cli*.jar  $p$b --keystore-password=$KEYSTORE_PASS  $m$opt --out=./release/$1-$2.apk$excludePatches$includePatches$ks $pu$force $a./download/$1.apk
  		unset version
		unset lock_version
...
split_arch() {
	...
	if [ -f "./download/$1.apk" ]; then
	    ...
	    --keystore-password=$KEYSTORE_PASS  --keystore-entry-alias=$KEYSTORE_ALIAS \
